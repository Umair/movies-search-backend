    ...
    ActiveRecord::Base.descendants.each do |model|
      if model.respond_to?(:__elasticsearch__)
        begin
          ...
          STDERR.puts "There was an error creating the elasticsearch index1 for #{model.name}: #{e.inspect}"
        end
      end
    end
  end
    ...
    ActiveRecord::Base.descendants.each do |model|
      if model.respond_to?(:__elasticsearch__)
        begin
          ...
          STDERR.puts "There was an error removing the elasticsearch index1 for #{model.name}: #{e.inspect}"
        end
      end
    end
  end
